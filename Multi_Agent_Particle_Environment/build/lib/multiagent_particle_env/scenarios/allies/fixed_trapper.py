...
from multiagent_particle_env.alternate_policies import trapper_fixed_strategy
from multiagent_particle_env.alternate_policies import double_pendulum_perturbation_strategy_2
...
class Scenario(BaseScenario):
    ...
    def make_world(self, args):
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.collide = True
            # TODO: Should this be reset to zero on world reset?
            agent.counter = 0
            agent.silent = True
            agent.size = 0.075 if agent.adversary else 0.05
            agent.max_speed = 1.0 if agent.adversary else 1.3

        for i in range(num_fixed_adv):
            if args.perturbation:
                self.adversaries(world)[i].action_callback = double_pendulum_perturbation_strategy_2
                self.adversaries(world)[i].is_perturbed_policy = True
                args.perturbation = False
            else:
                self.adversaries(world)[i].action_callback = trapper_fixed_strategy
                self.adversaries(world)[i].is_fixed_policy = True
    ...
    def reset_world(self, world):
        ...
        for landmark in world.landmarks:
            if not landmark.boundary:
                ...
                landmark.state.p_pos = np.zeros(world.dimension_position)
