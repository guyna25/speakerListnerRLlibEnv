...
def _fixed_strategy(agent, world):
    """
    Fixed strategy for changing an agent's policy
    ...
    prey = prey[0]
    prey_p = np.array(prey.state.p_pos)
    prey_me = np.array(agent.state.p_pos) - prey_p
    prey_v = np.array(prey.state.p_vel)
    prey_next = prey_p + prey_v
    prey_next_me = np.array(agent.state.p_pos) - prey_next

    # print(np.array(prey.state.p_vel))

    if np.linalg.norm(np.array(prey.state.p_vel)) == 0.0:
        vec = prey_me / np.linalg.norm(prey_me)
    else:
        vec = prey_me / np.linalg.norm(prey_me) - prey_v / np.linalg.norm(prey_v)
...
class Scenario(BaseScenario):
    ...
    def make_world(self, args):
        ...
        if args.perturbation:
            self.adversaries(world)[0].action_callback = _double_pendulum_perturbation_strategy
            self.adversaries(world)[0].is_perturbed_policy = True

        # Set fixed policy agents
        if args.fixed:
            self.adversaries(world)[0].action_callback = _fixed_strategy
            self.adversaries(world)[0].is_fixed_policy = True
    ...
    def reset_world(self, world):
        ...
        for agent in world.agents:
            ...
            if agent.is_perturbed_policy or agent.is_fixed_policy:
                agent.color = np.array([66. / 255, 185. / 255, 244. / 255])
