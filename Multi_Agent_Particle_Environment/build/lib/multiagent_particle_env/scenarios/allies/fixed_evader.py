...
from multiagent_particle_env.alternate_policies import evader_fixed_strategy
...
class Scenario(BaseScenario):
    ...
    def make_world(self, args):
        ...
        for i in range(num_fixed_prey):
            self.good_agents(world)[i].size = 0.075
            if args.perturbation:
                self.good_agents(world)[i].action_callback = double_pendulum_perturbation_strategy_2
                self.good_agents(world)[i].is_perturbed_policy = True
                args.perturbation = False
            else:
                self.good_agents(world)[i].action_callback = evader_fixed_strategy
                self.good_agents(world)[i].is_fixed_policy = True
