...
"""
two_on_one_tag_no_collision.py
...
from multiagent_particle_env.alternate_policies import double_pendulum_perturbation_strategy_3
...
class Scenario(BaseScenario):
    ...
    def make_world(self, args):
        ...
        num_good_agents = 1
        num_adversaries = 2
        num_fixed_adv = args.num_fixed
        # num_adversaries = num_adversaries - num_fixed_adv
        ...
        for i in range(num_fixed_adv):
            self.adversaries(world)[i].is_fixed = True
            self.adversaries(world)[i].action_callback = distance_minimizing_fixed_strategy
        ...
        if args.perturbation:
            self.adversaries(world)[0].action_callback = double_pendulum_perturbation_strategy_3
            self.adversaries(world)[0].collide = False
