...
from multiagent_particle_env.alternate_policies import sheep_fixed_strategy
...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        # Set properties for fixed agents
        for scripted_agent in world.scripted_agents:
            scripted_agent.color = np.array([0.58, 0.95, 0.26])

        # Set properties for landmarks
        for landmark in world.landmarks:
            landmark.color = np.array([0.25, 0.25, 0.25])

        # Set random initial states for policy agents
        ...
        # Set random initial states for scripted agents
        for agent in world.scripted_agents:
            ang = np.random.uniform(-2 * np.pi, 2 * np.pi)
            agent.state.p_pos = 0.25 * np.array([np.cos(ang), np.sin(ang)])
            agent.state.p_vel = np.zeros(world.dimension_position)
            agent.state.c = np.zeros(world.dimension_communication)
    ...
    def adversary_reward(self, agent, world):
        ...
        rew = 0
        sheep = world.scripted_agents
        give_reward = True
        for shp in sheep:
            rew -= np.linalg.norm(shp.state.p_pos)
            if np.linalg.norm(shp.state.p_pos) > 0.1:
                give_reward = False
        if give_reward:
            rew += 30
...
#                        rew += 10
    ...
    def adversary_reward(self, agent, world, shaped):
        ...
        give_reward = True
        for shp in sheep:
            reward -= np.linalg.norm(shp.state.p_pos)
            if np.linalg.norm(shp.state.p_pos) > 0.1:
                give_reward = False
            if give_reward:
                reward += 30
