...
def parse_args():
    ...
    # Evaluation
    parser.add_argument("--pred-0-network", type=int, default=None,
                        help="Predator 0 network to use for action inference")
    parser.add_argument("--pred-1-network", type=int, default=None,
                        help="Predator 1 network to use for action inference")
    parser.add_argument("--pred-2-network", type=int, default=None,
                        help="Predator 2 network to use for action inference")
...
def train(arglist):
    ...
    with tf_util.single_threaded_session():
        ...
        while True:
            ...
            # Get action
            if (arglist.pred_0_network is None) and (arglist.pred_1_network is None) and \
                    (arglist.pred_2_network is None):
                action_n = [agent.action(obs) for agent, obs in zip(trainers, obs_n)]
            else:
                action_n = []
                if arglist.pred_0_network is not None:
                    action_n.append(trainers[arglist.pred_0_network].action(obs_n[0]))
                else:
                    action_n.append(trainers[0].action(obs_n[0]))

                if arglist.pred_1_network is not None:
                    action_n.append(trainers[arglist.pred_1_network].action(obs_n[1]))
                else:
                    action_n.append(trainers[1].action(obs_n[1]))

                if arglist.pred_2_network is not None:
                    action_n.append(trainers[arglist.pred_2_network].action(obs_n[2]))
                else:
                    action_n.append(trainers[2].action(obs_n[2]))
