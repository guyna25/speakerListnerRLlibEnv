...
from maddpg.trainer.maddpg_ccm import MADDPGAgentTrainerCCM
...
def parse_args():
    ...
    #CCM
    parser.add_argument("--use-ccm", action="store_true", default=False, help="Flag for controlling use of CCM")
    parser.add_argument("--perturbation", action="store_true", default=False,
                        help="Flag for controlling perturbation analysis")
    parser.add_argument("--ccm-lambda", type=float, default=0.0, help="Lambda values for weighting CCM training")
    ...
    parser.add_argument("--specific-agent-ccm", type=int, default=None,
                        help="Identify a particular agent for CCM calculation")
    parser.add_argument("--specific-leader-ccm", type=int, default=None,
                        help="Identify a leader that all others coordinate with")
    parser.add_argument("--ccm-on-policy", action="store_true", default=False)
    parser.add_argument("--training-history", type=int, default=1,
                        help="Number of frames of agent history to include in training")
    parser.add_argument("--ccm-pool", type=int, default=None, help="Number of consecutive trials to use for CCM")
...
def train(arglist):
    ...
    with tf_util.single_threaded_session():
        ...
        while True:
            ...
            if arglist.use_ccm:
                for agent in trainers:
                    if arglist.ccm_lambda == 1.0:
                        if arglist.specific_agent_ccm is None:
                            loss = agent.ccm_update(trainers, train_step)
                        else:
                            if agent.agent_index == arglist.specific_agent_ccm:
                                loss = agent.ccm_update(trainers, train_step)
                            else:
                                arglist.ccm_lambda = 0.0
                                loss = agent.update(trainers, train_step)
                                arglist.ccm_lambda = 1.0
                    elif arglist.ccm_lambda == 0.0:
                        # Original MADDPG
                        loss = agent.update(trainers, train_step)
                    else:
                        loss = agent.update(trainers, train_step)
                        if arglist.specific_agent_ccm is None:
                            loss = agent.ccm_update(trainers, train_step)
                        elif agent.agent_index == arglist.specific_agent_ccm:
                            loss = agent.ccm_update(trainers, train_step)
