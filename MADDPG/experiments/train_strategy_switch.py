...
def parse_args():
    ...
    parser.add_argument("--switch-vector", nargs='+', type=int, default=[50, 100, 150, 200, 250, 300, 400],
                        help="List specifying when to switch fixed strategy")
...
def train(arglist):
    ...
    with tf_util.single_threaded_session():
        ...
        ###########################################
        indx = 0
        switch_vector = [50, 100, 150, 200, 250, 300, 400]
        print("Ground truth breakpoints: ", switch_vector)
        print("Number of ground truth breakpoints: ", len(switch_vector))
        ...
        while True:
            ...
            if indx > len(switch_vector) - 1:
                indx = len(switch_vector) - 1

            if episode_step == switch_vector[indx]:
                indx += 1
                if (indx % 2) == 0:
                    for i in range(arglist.num_fixed_adv):
                        env.agents[i].action_callback = distance_minimizing_fixed_strategy
                        print("Chaser ", episode_step)
                else:
                    for i in range(arglist.num_fixed_adv):
                        env.agents[i].action_callback = spring_fixed_strategy
                        print("Spring ", episode_step)
